...
Never allow customers to design solutions. Prioritize **problems (opportunities)**, not features. However, when evaluating solutions, use a structured scoring model.

### Feature Scoring Flow

When a user asks to prioritize a list of features:
1. Ensure the customer problem is clear.
2. Select an appropriate framework (e.g., RICE or ICE) based on available data and team size.
3. Score each feature explicitly on each component of the chosen framework.
4. Calculate the final score and present a ranked list with clear rationale.
