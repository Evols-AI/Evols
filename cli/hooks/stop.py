...
def load_config():
    # Env vars (EVOLS_* injected via plugin.json hooks env block, or CLAUDE_PLUGIN_OPTION_*
    # from the marketplace) always take precedence over config.json per-key.
    env_url = os.environ.get("EVOLS_API_URL") or os.environ.get("CLAUDE_PLUGIN_OPTION_EVOLS_API_URL", "")
    env_key = os.environ.get("EVOLS_API_KEY") or os.environ.get("CLAUDE_PLUGIN_OPTION_EVOLS_API_KEY", "")
    env_plan = os.environ.get("EVOLS_PLAN") or os.environ.get("CLAUDE_PLUGIN_OPTION_EVOLS_PLAN", "")
...
def main():
    ...
    lines = []
    if is_failure:
        lines.append("[Evols] Session ended due to rate limit.")
        lines.append(f"  Tokens used this session: {total_tokens:,}")
        lines.append(f"  5h block usage:  {block_tokens_total:,} / {block_limit:,}  ({block_pct}%  ·  resets in {block_remaining_h}h {block_remaining_m}m)")
        lines.append("  This event has been recorded for your team's quota tracking.")
    else:
        lines.append("[Evols] Session complete.")
        lines.append(f"  Model:          {model}")
        lines.append(f"  Input tokens:   {input_tokens:,}")
        lines.append(f"  Output tokens:  {output_tokens:,}")
        if cache_read_tokens:
            lines.append(f"  Cache reads:    {cache_read_tokens:,}  (discounted)")
        lines.append(f"  Session total:  {total_tokens:,}  ({round(session_minutes)}m)")
        if burn_rate_per_min >= 1.0:
            projected_block = int(burn_rate_per_min * BLOCK_HOURS * 60)
            lines.append(f"  Burn rate:      {burn_rate_per_min:.0f} tok/min  →  ~{projected_block:,} projected/block")
        lines.append(f"  Session cost:   ${session_cost_usd:.4f}")
        lines.append(f"  5h block usage: {block_tokens_total:,} / {block_limit:,}  ({block_pct}%  ·  resets in {block_remaining_h}h {block_remaining_m}m)")
        if tokens_retrieved > 0:
            ...
            lines.append(f"  Team context:   {tokens_retrieved:,} tokens retrieved  →  ~{display_savings:,} tokens saved vs. compiling fresh")
        if synced_entry_id:
            lines.append(f"  Knowledge sync: entry #{synced_entry_id} added to team graph  (investment: {tokens_created:,} tokens)")
            lines.append("                  Savings realized when you or teammates retrieve this context in future sessions.")
        else:
            lines.append("  Knowledge sync: call sync_session_context to share insights with your team")

    summary = "\n".join(lines)
    # Write human-readable summary to stderr (visible in terminal, not parsed by agents)
    print(summary, file=sys.stderr)

    # Output valid JSON to stdout — required by Codex and other agents that parse hook output
    print(json.dumps({"continue": True, "stopReason": "evols_stop_hook"}))
