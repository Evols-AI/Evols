...
![Product manager reviewing a polished-looking AI-generated spec document, unaware it lacks key decisions](https://cdn-images-1.medium.com/max/1600/0*RXQpHB-5qjkM4jZF)
...
![Engineering team discovering during implementation that a design document missed critical system-specific constraints](https://cdn-images-1.medium.com/max/1600/0*AUkaaGc6CC1vMIcl)
...
The 2025 Stack Overflow Developer Survey, covering 49,000 respondents, found that 66% of developers struggle with "AI solutions that are almost right, but not quite." The documentation problem is the written analog of this: not wrong enough to reject immediately, but wrong in ways that become apparent when you try to use it.
...
I use the following six questions to distinguish documents that can be acted on from documents that look like they can be acted on.
...
"This document covers the authentication service redesign" is different from "This document covers the token storage and validation logic; it does not cover session management, which is addressed in a separate document, or the rate limiting layer, which will be designed separately." The first statement doesn't help the reader understand what falls outside this document. The second does.

Scope ambiguity is expensive. Work gets done by two people because neither was sure who owned it. Integration points get missed because each person assumed the other document covered them. AI-generated documents tend toward implied scope because explicit scope requires knowing what else is in play, context the model often doesn't have.
...
![A reviewer making light edits to a polished document without questioning whether the underlying decisions are present](https://cdn-images-1.medium.com/max/1600/0*hLwCxx1toH2w6uJ2)
...
![Developer and PM sitting together to clarify unresolved decisions before finishing a design document with AI assistance](https://cdn-images-1.medium.com/max/1600/0*4gPucGRu77Go7OJJ)
